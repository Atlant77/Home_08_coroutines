...
//                    .map { post ->
//                        PostWithComments(post, getComments(client, post.id), post.authorId)
//                    }
//                println(posts)
//            } catch (e: Exception) {
//                e.printStackTrace()
    ...
    with(CoroutineScope(EmptyCoroutineContext)) {
        launch {
            try {
                ...
                println(posts)
                println("List of received posts:")
                posts.map {
                    println(it)
                }
            } catch (e: Exception) {
...
suspend fun getAuthor(client: OkHttpClient, id: Long): Author =
    makeRequest("$BASE_URL/api/slow/posts/author/$id", client, object : TypeToken<Author>() {})
